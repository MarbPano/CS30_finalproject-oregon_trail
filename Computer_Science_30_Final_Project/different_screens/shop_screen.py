...
def shop(screen, width, height, money):
    ...
    clothing_button_minus_rect = clothing_button_minus.rect
        ...
        #actual button - continue button
    continue_button = wc.Button(width/3+30, 550, 220, 120, button_border, button_color, screen)
...
######-----------#######
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                if not confirm_active:
                    ...
                    #food
                    elif food_button_plus_rect.collidepoint(event.pos):
                        if money < food_price:
                            None
        ...
        #drawing it out - continue
        continue_button.draw(screen)
        continue_button.with_text('Continue','Corbel', 50, (255, 255, 255), screen, bold=True)
        continue_button.update()

        if confirm_active:
            confirm_box = wc.Confirm(screen)
            confirm_box.create(screen)
            confirm_choice = confirm_box.collide(screen)
